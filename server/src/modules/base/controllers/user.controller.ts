...
import {
  ...
  AddUserDto,
  UpdateUserDto,
  BatchSetUserPositionDto,
  BatchSetUserRolesDto,
  ImportUsersDto,
} from '../dto/base.dto';
...
export class UserController extends CrudControllerFactory(UserVo) {
   ...
   * 批量设置用户岗位
   */
  @Post('batch-set-position')
  @Perms('batch-set-position')
  @ApiOperation({ summary: '批量设置用户岗位' })
  @ApiOkVoid()
  async batchSetPosition(@Body() dto: BatchSetUserPositionDto) {
    await this.userService.batchSetPosition(dto.ids, dto.positionId);
    return this.ok();
  }
  ...
  @Perms('batch-set-roles')
  @ApiOperation({ summary: '批量设置用户角色' })
  @ApiOkVoid()
  async batchSetRoles(@Body() dto: BatchSetUserRolesDto) {
    await this.userService.batchSetRoles(dto.ids, dto.roleIds);
    return this.ok();
  }

  /**
   * 导入用户
   * 前端解析 Excel 后提交结构化行数据，后端统一完成密码哈希和用户创建。
   */
  @Post('import')
  @Perms('import')
  ...
  async importUsers(@Body() dto: ImportUsersDto) {
    const rows = await Promise.all(
      dto.rows.map(async (row) => ({
        ...row,
        password: await this.authService.hashPassword(row.password),
      })),
    );
    return this.ok(await this.userService.importUsers(rows));
  }
  ...
  async exportUsers(@Query() query: { keyword?: string; status?: string; departmentId?: string }) {
    const rows = await this.userService.listForExport(query);
    const header = ['ID', '账号', '姓名', '手机号', '邮箱', '部门', '岗位', '角色', '状态', '创建时间'];
    const body = rows.map((row) => [
      ...
      row.department?.name || '',
      row.position?.name || '',
      row.userRoles.map((ur) => ur.role.name).join(';'),
      row.status === 1 ? '启用' : '禁用',
