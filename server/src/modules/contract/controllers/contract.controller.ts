...
import { ApiOkVoid, ApiPageResult, ApiResult, Perms } from '@/common/decorators';
...
export class ContractController extends CrudControllerFactory(ContractVo) {
  ...
  async list(@Query() query: PageOptions & Record<string, any>) {
    return this.ok(await this.contractService.pageContracts(query));
  }
  ...
  async add(@Body() dto: AddContractDto) {
    return this.ok(await this.contractService.createContract(dto));
  }
  ...
  @ApiResult(ContractVo)
  async update(@Body() dto: UpdateContractDto) {
    return this.ok(await this.contractService.updateContract(dto));
  }
  ...
  async export(@Query() query: Record<string, any>) {
    const rows = await this.contractService.listForExport(query);
