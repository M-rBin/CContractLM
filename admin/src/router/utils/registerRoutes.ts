...
function handleIframeRoute(
  ...
  targetRoute.component = () =>
    Promise.resolve({
      render() {
        return h('div', { style: 'padding: 20px' }, 'iframe 功能已移除')
      }
    })
}
...
function handleLayoutRoute(
  ...
  converted.name = route.name ? `${String(route.name)}Layout` : ''
  route.meta.isFirstLevel = true

  const childComponent = typeof component === 'function'
    ? component
    : loadComponent(component as string, String(route.name))
...
function handleNormalRoute(
  ...
  if (component) {
    if (typeof component === 'function') {
      ...
      converted.component = loadComponent(component, routeName)
    }
  }
}
