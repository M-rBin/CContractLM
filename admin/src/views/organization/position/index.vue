...
<script setup lang="ts">
  ...
  function handleEdit(row: any) {
    const position = row as PositionItem
    Object.assign(form, {
      id: position.id,
      name: position.name,
      description: position.description || '',
      orderNum: position.orderNum
    })
  ...
  async function handleDelete(row: any) {
    const position = row as PositionItem
    try {
      await ElMessageBox.confirm(`确定要删除岗位"${position.name}"吗？`, '提示', { type: 'warning' })
  ...
  let isMounted = false
  onMounted(() => { isMounted = true; loadList() })
  onActivated(() => { if (isMounted) loadList() })
</script>
