...
<script setup lang="ts">
  ...
  async function handleDelete(row: any) {
    const department = row as Department
    try {
      await ElMessageBox.confirm(`确定要删除部门"${department.name}"吗？`, '提示', { type: 'warning' })
  ...
  let isMounted = false
  onMounted(() => { isMounted = true; loadDepartmentList() })
  onActivated(() => { if (isMounted) loadDepartmentList() })
</script>
